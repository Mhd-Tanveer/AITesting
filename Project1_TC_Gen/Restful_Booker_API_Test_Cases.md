...
- **Summary**: Verify deleted booking cannot be retrieved after deletion
- **Preconditions**: API server is running with valid auth token and a booking to delete
- **Steps to Reproduce**:
  1. Create a test booking and note the booking ID
  2. Create valid auth token
  3. Send DELETE request to `https://restful-booker.herokuapp.com/booking/<booking_id>`
  4. Set Cookie header with valid token
  5. Verify deletion success (HTTP 201)
  6. Send GET request to `https://restful-booker.herokuapp.com/booking/<booking_id>`
  7. Verify response indicates booking no longer exists
- **Expected Result**:
  - DELETE request returns HTTP Status Code: 201 Created
  - GET request returns HTTP Status Code: 404 Not Found
  - Booking is no longer accessible in the system
- **Actual Result**: 
- **Status**: 

### TC_BOOKING_DELETE_008 - Delete Booking - Delete with Expired Token
- **Summary**: Attempt to delete booking with expired/invalid token
- **Preconditions**: API server is running with an expired authentication token
- **Steps to Reproduce**:
  1. Create a valid auth token
  2. Wait for token to expire or use an outdated token
  3. Send DELETE request to `https://restful-booker.herokuapp.com/booking/1`
  4. Set Cookie header with expired token
  5. Observe error response
- **Expected Result**:
  - HTTP Status Code: 403 Forbidden or 401 Unauthorized
  - Response indicates token is expired or invalid
- **Actual Result**: 
- **Status**: 

### TC_BOOKING_DELETE_009 - Delete Booking - Negative ID
- **Summary**: Attempt to delete booking with negative ID
- **Preconditions**: API server is running, valid auth token available
- **Steps to Reproduce**:
  1. Create valid auth token
  2. Send DELETE request to `https://restful-booker.herokuapp.com/booking/-1`
  3. Set Cookie header with valid token
  4. Observe error response
- **Expected Result**:
  - HTTP Status Code: 404 Not Found or 400 Bad Request
...
### TC_BOOKING_DELETE_010 - Delete Booking - Delete Already Deleted Booking
- **Summary**: Attempt to delete a booking that was already deleted
- **Preconditions**: API server is running with a booking ID that was previously deleted
- **Steps to Reproduce**:
  1. Create a test booking and note the booking ID
  2. Create valid auth token
  3. Send DELETE request to the booking
  4. Verify deletion success
  5. Send second DELETE request to same booking ID
  6. Observe error response
- **Expected Result**:
  - First DELETE request returns HTTP Status Code: 201 Created
  - Second DELETE request returns HTTP Status Code: 404 Not Found
  - Response indicates booking was already deleted or doesn't exist
- **Actual Result**: 
- **Status**: 
...
| Section | Test Cases | Test IDs |
|---------|-----------|----------|
| PING Endpoint | 2 | TC_PING_001 - TC_PING_002 |
| AUTH Endpoint | 7 | TC_AUTH_001 - TC_AUTH_007 |
| GET Booking IDs | 9 | TC_BOOKING_IDS_001 - TC_BOOKING_IDS_009 |
| GET Booking | 8 | TC_BOOKING_GET_001 - TC_BOOKING_GET_008 |
| CREATE Booking | 15 | TC_BOOKING_CREATE_001 - TC_BOOKING_CREATE_015 |
| UPDATE Booking | 8 | TC_BOOKING_UPDATE_001 - TC_BOOKING_UPDATE_008 |
| PATCH Booking | 6 | TC_BOOKING_PATCH_001 - TC_BOOKING_PATCH_006 |
| DELETE Booking | 10 | TC_BOOKING_DELETE_001 - TC_BOOKING_DELETE_010 |

---

## Test Case Execution Template

For each test case, use the following JIRA format:
...
Test Type: [Functional/Regression/Sanity]
...
Defect Reference: [If failed, link to JIRA defect]
...
1. **Environment Setup**: Ensure API is running at https://restful-booker.herokuapp.com
2. **Test Data**: Use consistent test data to ensure reproducibility
3. **Authentication**: Always create a fresh auth token before executing protected endpoints
4. **Logging**: Document all responses, including response headers and body
5. **Error Handling**: Pay special attention to HTTP status codes and error messages
6. **Timing**: Note response times to identify performance issues
7. **Dependencies**: Some tests depend on results of previous tests (e.g., DELETE depends on CREATE)

---

## Related Documents

- [TEST_PLAN.md](TEST_PLAN.md) - Comprehensive test plan document
- Restful-Booker API Official Documentation
- Postman Collection (if available)
- Test Execution Report (to be created after execution)
